...
New function contract: `prepareModelTurnMessages(input: { messages: ChatMessage[]; conv: ConversationRuntime; model: ModelConfig; config: ForgeConfig; activeFile: string | undefined; turnContext: TurnContextState; checkpoint?: CheckpointSession; isVisionModel: boolean; templateContext: TemplateContext; forgeInstructions?: string; }) : ChatMessage[]`. Before implementing, inspect each exact free variable and remove unused fields; pass narrow values, not `ModelTurnContext` or an opaque context bag. If actual pipeline has materially different inputs, stop and revise this plan first. Keep `runModelTurn(ctx, request)` and its exports stable.
