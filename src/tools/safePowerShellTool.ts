...
async function resolveReadOnlyPath(
  ...
  const resolved = await resolveRealWorkspacePath(pathValue, root, { relativeOnly: true });
        ...
        parameters: {
          ...
          properties: {
            ...
            path: {
              type: 'string',
              description:
                'Workspace-relative existing path. Required for list_directory and get_file_hash.',
            },
            max_entries: {
              type: 'integer',
              minimum: 1,
