...
**Phase 3, attempt 2:** stopped by Claude after 84+ calls and no edit (F5). The chat had grown to ~92K prompt tokens per request; the model config notes Qwen degrades past ~100K. Both attempts ran in the phase 1 chat, so neither is a fair read of Qwopus. **Owner, 2026-09-22: Qwopus is evaluated (and fixed, if fixable) separately, outside this run. Phase 3 onward is written by Qwen Q6 (`qwen38-27b-dflash2-ud-q6k-vision`, thinking on) in the same, compacted chat.** Qwopus's ctx was cut to 75K in `.forge/config.yaml` for that separate evaluation.
...
| # | Found | Fix | Status |
|---|---|---|---|
| F1 | Phase 3 attempt 1: Qwopus ran the same two `holdAwake` searches 16 times; `ToolLoopGuard` threw with no warning, so the model never saw it was repeating. The system-prompt rule against repeats already existed and did not help | On the first exact repeat of a read-only call with an identical result, prefix the result with one line naming the earlier call and telling it to act or change approach; keep the throw for persistent loops | **done** `9f509be` |
| F2 | `forge.sh send claude codex` → `unknown recipient "codex"`: the alias is written with `session_id ""`, `readAliases` (`src/agentMesh/aliasRegistry.ts`) drops it, `knownAliases` (`src/vscode/agentMeshSetup.ts`) omits codex | Register the owned Codex under a routable id, or let `knownAliases` include owned sessions without one | **done** `79843de` |
| F3 | A bus-started turn (`forge.sh say`) was not mirrored to Telegram | Treat bus turns as a remote origin for the mirror | **not reproduced yet.** The binding is on the same conversation, and mirroring is on unless `/mirror off` is sent. Codex's patch made bus turns override `/mirror off`; it was rejected and parked at `~/.forge/agent-bus/outbox/codex-f3-rejected.patch`. Needs a live repro |
| F4 | `/agent/message` lands in the active chat on its model: no way to start a phase on a chosen model **or in a fresh chat**. Phase 3 attempt 2 ran in the phase 1 chat at ~92K prompt tokens per request (131K window) and degraded | Optional `model` and `new_chat` on `/agent/message` + `forge.sh say --model <m> --new` | **done** `16214b1` |
| F5 | Phase 3 attempt 2: 84+ calls, no edits; `read_file` paged 100-line windows up to line 3600 of two ~497-line files, ignoring "start_line 2400 is past end_line 498 (file has 498 lines)" every time. Each error differs, so F1's identical-result check cannot see it | Count consecutive failures of the same tool on the same path; from the 3rd, prefix the result with a stop line naming the count and the file length | **done** `c13c50f`; Claude fixed a bypass of the hard stops in `1fef9a3` |
| F6 | `forge.sh send claude codex` said "not delivered" (400 `illegal exchange transition started -> accepted`) although Codex received and answered: an idle recipient starts draining on enqueue, before the relay wrote its hop-2 `accepted` | Record hop 2 before the enqueue | **done** `90cfeaa` |
| F7 | After a VS Code reload Qwen's `ask_live_session` to `claude` was answered by a Forge-spawned Claude, not the user's session: the join was keyed by pid, and a reload restarts the panel session under a new pid only once its panel is opened | Match the join by Claude's `sessionId` too; a joined session that is not running is refused, never replaced | **done** `09c76a4` |
| F8 | The owned Codex (and owned Claude) ownership record kept `session_id ""` forever: it is written at creation, before a fresh session has an id. Every reload logged "thread kept for resume" and started an empty thread | Record the confirmed id when an owned turn ends | **done** `09c76a4` |
