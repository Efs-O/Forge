...
| `jobs/staged/<id>.json` | `stagedBuild` | **✗ NOT removed — audit F1** (fixed: `delete` clears it; a stage that begins after the delete, from a check already in flight, is refused, and `processPendingSwitches` drops any record whose job is gone — never switched) | kept (a paused job can still switch) | atomic; corrupt ⇒ treated as absent | kept; next holder switches it | `STAGE_TTL_MS` 24 h, checked in `performSwitch` + `approveStaged` |
...
| `ForgeScheduledWake` (Task Scheduler) | `PowerControl` via `WakeReconciler` (lease holder only) | reconciled on the store watch; re-registered only when the computed schedule changed (audit A4, 2026-09-21) | reconciled (a disabled job contributes no wake) | n/a (Windows owns it) | **✗ survives a disposed scheduler — keeps waking the PC for jobs nobody runs.** A jobs-disabled window deletes it only after taking the scheduler lease, so it never deletes a live owner's task (`clearWakesIfUnowned`, audit A7) | none; `[]` deletes the task |
| `state.summary_failures` / `state.summary_retry_at` (in `jobs/state/<id>.json`) | `JobDelivery` | removed with the state file | kept; a paused job's pending summary does no model work (audit A8) | same atomic state write; a missing field defaults to `0` / `null` | kept; the next holder waits until `summary_retry_at`, then retries | backoff 1 min doubling to a 6 h cap; the outbox reports the first failure only; both reset on success (audit A9) |
